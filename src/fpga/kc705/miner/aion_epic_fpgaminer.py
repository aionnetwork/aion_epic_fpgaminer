...
USER_AGENT = "aion_epic_fpgaminer"
VERSION = [0, 1]
...
coloredlogs.install(level='DEBUG', fmt='%(asctime)s, %(message)s',field_styles={'asctime':{'color':'magenta'}})

#Open the device
def open_device(port):
	return serial.Serial(port, 57600, serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE, 1, False, False, 5, False, None)
...
def read_message(device):
    if device:
		return device.read_until()


def convert_to_temp(raw_temp):
    temp = (int(raw_temp,16)*503.975/65536)-273.15
    return temp
...
class Subscription(object):
  ...
  def set_difficulty(self, difficulty):
    ...
    if difficulty == 0:
      ...
      target = min(int((0xffff0000 * 2 ** (256 - 64) + 1) / int(difficulty) - 1 + 0.5), 2 ** 256 - 1)
...
class Miner(SimpleJsonRpcClient):
  ...
  def handle_reply(self, request, reply):
    ...
    if reply.get('method') == 'mining.notify':
      if 'params' not in reply or len(reply['params']) != 4:
    ...
    # This is a reply to...
      ...
      if request.get('method') == 'mining.subscribe':
        if 'result' not in reply or len(reply['result']) != 2 :#emai3 or len(reply['result'][0]) != 2:
        ...
        ( extranounce1, extranounce2_size) = reply['result']
        subscription_id = 1234
        #print "[ExtraNonce] " + extranounce2_size
        self._nonce = extranounce2_size

        self._subscription.set_subscription(subscription_id , extranounce2_size,extranounce1)
  ...
  def serve_forever(self):
    ...
    ePIC_COM_PORT = "COM1"
    ePic_miner = open_device(ePIC_COM_PORT)
    message = self._header.encode('ascii','ignore') + self._nonce.encode('ascii','ignore') + "0000000000000000000000000000000000000000"
    send_message(ePic_miner,message)
    while True:
      message = read_message(ePic_miner)
      tmp = message.rstrip().decode('hex')
      if len(tmp) == 2:
        temp = convert_to_temp(message)
        logger.debug("This is the temp %s" % temp)
      elif len(tmp) == 4:
        #end_time = time()
        total_time = 0
        if (int(temp,32)==0xDEADBEEF):
          for i in range (512):
            message = read_message(ePic_miner)
            logger.info( "Index[%x] : %x" % (i,int(message.rstrip(),32)))
      else:
        pass
...
ePic_logo = """
               ...
               `+mNdddddddddddNNMs
             `omNddddddddddmNy-+M.
            :NMmmdddddddddmmy-   mh                       ++++++++/:.`     :+`       `.:/ooo+/.`
         `` .Nh:+syddmmmmms.     :M:                      MMoooooosymdo`   hM:     .odmhs+++oyddy:
       -smdo oN. `` `.-/Ns        hd       `-://:-.       MM        `/Nm`  hM:   `sNh:`        -o+
     -yd+sN- `my +yys:  sN`      .yN-    -yddyoosddh/`    MM          sMo  hM:  `hM+
   :yh/` `Ns  /M- `.ym` `Ns    .sdo.    sNh-      .sMh.   MM         `hM:  hM:  /My
`:hh/`    +M-  dd:-`.No  +M. -sdo.     +Mh.        `sMy   MM.......-+dm+   hM:  sM+
.Ns`       dh  ./os+ oN. :Nhyd+`       yMmhhhhhhhhhhdmy   MMhhhhhhhhy+.    hM:  :Mh
 ym`       sMs+/-.`  `my /yh/`         /Md                MM````````       hM:   sMy`
 .No    `/dmddmmmmddyshM-               oNd:`     `:yo    MM               hM:    /dmo-`      .+ds
  sN` `+dmdddddddddddmMh-                -sdmhsoshdh+`    MM               hM:     `:shmdhyyhmdh+.
  `Ny+mmdddddddddddmNy-                    `.-:/:-``      ..               .-`        ``.-::-.`
   /NMNmmddddddddmNy-
    ``-/oshdNNmmNs.
    """
...
# Be fancy and print the logo
print(colored(ePic_logo, 'yellow'))
